...
import { loadResolvedConfig, prepareConfigResourcesForDeploy } from '../../../src/config'
import {
...
describe('config resource resolution', () => {
	...
	test('throws a helpful error when a named Hyperdrive configuration cannot be found', async () => {
		await expect(
			...
			'Could not find Hyperdrive configuration(s) for POSTGRES → missing-hyperdrive'
		)
	})
