...
describe('runCli', () => {
	test('returns exit code 0 for help', async () => {
		...
		expect(result.output).toContain(
			'previews — Inspect and clean dedicated preview Workers and scopes'
		)
		expect(result.output).toContain(
			'productions — Inspect and manage live production Workers and deployments'
		)
	...
	test('shows deploy help with explicit preview targeting syntax', async () => {
		...
		expect(result.output).toContain(
			'--preview <name> — Deploy a named preview scope such as `next` or `pr-1`'
		)
	...
	test('shows preview cleanup help', async () => {
		...
		expect(result.output).toContain(
			'--scope <name> — Clean one preview scope instead of the default `preview` scope'
		)
	...
	test('shows the same detailed help for `help <command>` and `<command> --help`', async () => {
		...
		expect(viaFlag.output).toContain(
			'devflare previews Inspect and clean dedicated preview Worker scopes'
		)
		expect(viaFlag.output).toContain(
			'devflare previews cleanup [--config <path>] [--env <name>] [--scope <name> | --all] [--account <id>] [--apply]'
		)
		...
		expect(viaFlag.output).toContain(
			'`cleanup` removes preview-only Cloudflare resources for the targeted scope and also deletes dedicated preview-scope Worker scripts'
		)
	})
	...
	test('shows nested help for preview cleanup', async () => {
		...
		expect(result.exitCode).toBe(0)
		expect(result.output).toContain(
			'devflare previews cleanup Delete preview-only Worker scripts and preview-scoped Cloudflare resources'
		)
		expect(result.output).toContain(
			'--scope <name> — Clean one preview scope instead of the default `preview` scope'
		)
		expect(result.output).toContain(
			'--all — Clean every live preview scope Devflare can discover for the current worker family'
		)
		expect(result.output).toContain('--apply — Apply the cleanup instead of doing a dry run')
		expect(result.output).toContain(
			'Dedicated preview Worker scripts are candidates only when their names resolve to the targeted preview scope'
		)
	})
	...
	test('shows nested help for worker rename', async () => {
		...
		expect(result.exitCode).toBe(0)
		expect(result.output).toContain(
			'devflare worker rename Rename a Worker and sync the matching config'
		)
		expect(result.output).toContain(
	...
	test('provides detailed help pages for every top-level command', async () => {
		const commandChecks = [
			...
			{
				argv: ['build', '--help'],
				snippet: 'devflare build Build production deployment artifacts'
			},
			{
				argv: ['deploy', '--help'],
				snippet: 'devflare deploy Deploy explicitly to Cloudflare production or preview targets'
			},
			{
				argv: ['types', '--help'],
				snippet: 'devflare types Generate TypeScript bindings from your config'
			},
			{ argv: ['doctor', '--help'], snippet: 'devflare doctor Check project configuration' },
			{
				argv: ['config', '--help'],
				snippet: 'devflare config Print resolved Devflare or Wrangler config'
			},
			{
				argv: ['account', '--help'],
				snippet: 'devflare account Inspect Cloudflare accounts, resources, and usage data'
			},
			{
				argv: ['login', '--help'],
				snippet: 'devflare login Authenticate with Cloudflare via Wrangler'
			},
			{
				argv: ['previews', '--help'],
				snippet: 'devflare previews Inspect and clean dedicated preview Worker scopes'
			},
			{
				argv: ['productions', '--help'],
				snippet: 'devflare productions Inspect and manage live production Workers and deployments'
			},
			{
				argv: ['worker', '--help'],
				snippet: 'devflare worker Rename and manage Worker control-plane operations'
			},
			{
				argv: ['tokens', '--help'],
				snippet: 'devflare tokens Manage Devflare-managed Cloudflare API tokens'
			...
			{
				argv: ['remote', '--help'],
				snippet: 'devflare remote Manage remote test mode for paid Cloudflare features'
			},
			{
				argv: ['help', 'help'],
				snippet: 'devflare help Show command overview or command-specific help'
			},
			{
				argv: ['version', '--help'],
				snippet: 'devflare version Show the installed devflare version'
			}
		]
	...
	test('provides detailed help pages for nested command paths', async () => {
		const nestedChecks = [
			{
				argv: ['config', 'print', '--help'],
				snippet: 'devflare config print Print the resolved config'
			},
			{
				argv: ['account', 'info', '--help'],
				snippet: 'devflare account info Show the selected account overview'
			},
			{
				argv: ['account', 'workers', '--help'],
				snippet: 'devflare account workers List Workers in the selected account'
			},
			{
				argv: ['account', 'kv', '--help'],
				snippet: 'devflare account kv List KV namespaces in the selected account'
			},
			{
				argv: ['account', 'd1', '--help'],
				snippet: 'devflare account d1 List D1 databases in the selected account'
			},
			{
				argv: ['account', 'r2', '--help'],
				snippet: 'devflare account r2 List R2 buckets in the selected account'
			},
			{
				argv: ['account', 'vectorize', '--help'],
				snippet: 'devflare account vectorize List Vectorize indexes in the selected account'
			},
			{
				argv: ['account', 'usage', '--help'],
				snippet: 'devflare account usage Show Devflare usage summaries'
			},
			{
				argv: ['account', 'limits', '--help'],
				snippet: 'devflare account limits Show or update Devflare usage limits'
			},
			{
				argv: ['account', 'limits', 'set', '--help'],
				snippet: 'devflare account limits set Set one Devflare usage limit'
			},
			{
				argv: ['account', 'limits', 'enable', '--help'],
				snippet: 'devflare account limits enable Enable Devflare usage-limit enforcement'
			},
			{
				argv: ['account', 'limits', 'disable', '--help'],
				snippet: 'devflare account limits disable Disable Devflare usage-limit enforcement'
			},
			{
				argv: ['account', 'global', '--help'],
				snippet: 'devflare account global Choose the global default Cloudflare account'
			},
			{
				argv: ['account', 'workspace', '--help'],
				snippet: 'devflare account workspace Choose the workspace Cloudflare account'
			},
			{
				argv: ['previews', 'list', '--help'],
				snippet: 'devflare previews list List stable workers and dedicated preview scopes'
			},
			{
				argv: ['previews', 'bindings', '--help'],
				snippet:
					'devflare previews bindings Inspect resolved bindings/resources and live worker associations'
			},
			{
				argv: ['previews', 'cleanup', '--help'],
				snippet:
					'devflare previews cleanup Delete preview-only Worker scripts and preview-scoped Cloudflare resources'
			},
			{
				argv: ['productions', 'list', '--help'],
				snippet:
					'devflare productions list List live production Workers and their active deployments'
			},
			{
				argv: ['productions', 'versions', '--help'],
				snippet:
			...
			{
				argv: ['productions', 'delete', '--help'],
				snippet: 'devflare productions delete Delete a live production Worker script'
			},
			{
				argv: ['worker', 'rename', '--help'],
				snippet: 'devflare worker rename Rename a Worker and sync the matching config'
			},
			{
				argv: ['remote', 'status', '--help'],
				snippet: 'devflare remote status Show the current effective remote-mode status'
			},
			{
				argv: ['remote', 'enable', '--help'],
				snippet: 'devflare remote enable Enable remote test mode'
			},
			{
				argv: ['remote', 'disable', '--help'],
				snippet: 'devflare remote disable Disable remote test mode'
			}
		]
