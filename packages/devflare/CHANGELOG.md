...
- cb15875: Report a deploy as successful only when something OBSERVED the upload, so a run that uploaded
  ...
  `DEVFLARE_VERIFY_DEPLOYMENT` still opts in to the extra control-plane round-trips _on top_ of it
...
- 437a6f9: Return the app worker's answer to a WebSocket upgrade verbatim, so a route that REFUSES one refuses
...
- c5ab77e: Carry `namespaceId` through to Miniflare, so a config declaring `bindings.rateLimits` can start a
