import { createRemoteSubcommandPage, entry } from '../shared'
...
export const MISC_HELP_PAGES: HelpPage[] = [
	{
		...
		options: [
			...
			entry(
				'--config <path>',
				'Choose the config file to update when multiple configs might match'
			),
			entry('--account <id>', 'Use a specific Cloudflare account when renaming the remote Worker')
		],
		examples: [
			entry(
				'devflare worker rename documentation --to devflare-documentation',
				'Rename a Worker and sync the matching config'
			)
		],
		notes: [
	...
	},
	{
		...
		examples: [
			entry(
				'devflare worker rename docs --to devflare-docs',
				'Rename the Worker and sync the selected config'
			)
		]
	},
	{
		...
		arguments: [
			entry(
				'<bootstrap-token>',
				'Account-owned bootstrap token with Cloudflare API token-management permissions'
			)
		],
		options: [
			...
			entry(
				'--all-flags',
				'With `--new`, include every reusable account/zone-scoped permission group'
			)
		],
		examples: [
			entry('devflare tokens $BOOTSTRAP --list', 'List managed tokens'),
			entry(
				'devflare tokens $BOOTSTRAP --new preview',
				'Create a managed token named `devflare-preview`'
			),
			entry('devflare tokens $BOOTSTRAP --roll preview', 'Roll the secret for `devflare-preview`'),
			entry(
				'devflare tokens $BOOTSTRAP --delete-all',
	...
	},
	{
		...
		subcommands: [
			...
			entry(
				'enable [minutes]',
				'Enable remote mode for a bounded duration (defaults to 30 minutes)'
			),
