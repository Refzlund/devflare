import { ACCOUNT_OPTION, createAccountInventoryPage, entry } from '../shared'
...
export const ACCOUNT_HELP_PAGES: HelpPage[] = [
	...
	createAccountInventoryPage(
		'info',
		'Show the selected account overview',
		'Displays the selected account plus shortcuts for other `account` subcommands.',
		'Show the account overview and suggested follow-up commands'
	),
	createAccountInventoryPage(
		'workers',
		'List Workers in the selected account',
		'Prints Worker names and last-modified timestamps for the resolved account.',
		'List Worker scripts in the selected account'
	),
	createAccountInventoryPage(
		'kv',
		'List KV namespaces in the selected account',
		'Prints namespace names and ids for the resolved account.',
		'List KV namespaces for the selected account'
	),
	createAccountInventoryPage(
		'd1',
		'List D1 databases in the selected account',
		'Prints database names, ids, and table counts for the resolved account.',
		'List D1 databases for the selected account'
	),
	createAccountInventoryPage(
		'r2',
		'List R2 buckets in the selected account',
		'Prints bucket names, creation dates, and locations for the resolved account.',
		'List R2 buckets for the selected account'
	),
	createAccountInventoryPage(
		'vectorize',
		'List Vectorize indexes in the selected account',
		'Prints Vectorize index names, dimensions, and metrics for the resolved account.',
		'List Vectorize indexes for the selected account'
	),
	createAccountInventoryPage(
		'usage',
		'Show Devflare usage summaries',
		'Prints Devflare-tracked usage totals and the currently configured usage limits.',
		'Show usage summaries for the selected account'
	),
	{
		path: ['account', 'limits'],
	...
	},
	{
		...
		examples: [
			...
			entry(
				'devflare account limits set ai-tokens 5000 --account <id>',
				'Set the daily AI token limit for a specific account'
			)
		],
		notes: [
	...
	},
	{
		...
		examples: [
			entry(
				...
				'Enable usage-limit enforcement for the resolved account'
			)
		],
		notes: [
	...
	},
	{
		...
		examples: [
			entry(
				...
				'Disable usage-limit enforcement for the resolved account'
			)
		],
		notes: [
