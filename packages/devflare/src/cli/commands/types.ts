...
import {
	...
	generateImportPath,
	resolveReferencedConfigs
} from './type-generation/discovery'
import { generateBindingTypes } from './type-generation/generator'
import type { DiscoveredDO } from './type-generation/models'

function logTypesLine(logger: ConsolaInstance, message = ''): void {
		...
		if (config.bindings?.durableObjects) {
			for (const [bindingName, doConfig] of Object.entries(config.bindings.durableObjects)) {
				...
				if (
					...
					(normalized.scriptName.endsWith('.ts') || normalized.scriptName.endsWith('.js'))
				) {
		...
		if (referencedConfigs.length > 0) {
			...
			for (const ref of referencedConfigs) {
				const typedBindings = ref.serviceBindings.filter(
					(serviceBinding) => serviceBinding.interfaceType
				)
				if (typedBindings.length > 0) {
					logTypesLine(
						logger,
						`  • ${ref.varName}: ${typedBindings.map((serviceBinding) => `${serviceBinding.bindingName} → ${serviceBinding.interfaceType}`).join(', ')}`
					)
				}
			}
		}
