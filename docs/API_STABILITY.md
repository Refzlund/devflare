...
| `devflare/vite` | `src/vite/index.ts` | Vite plugin: `devflarePlugin` (also default), `getPluginContext`, `getCloudflareConfig`, `getDevflareConfigs`, and the config-file helpers (`hasInlineViteConfig`, `resolveEffectiveViteProject`, `resolveViteUserConfig`, `writeGeneratedViteConfig`). |
