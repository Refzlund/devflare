...
function getInputEnvironmentKeys(name) {
	...
	return [...new Set([`INPUT_${normalizedName}`, `INPUT_${normalizedName.replace(/-/g, '_')}`])]
}
...
function buildCommentBody(config) {
	...
	const detailLines = []
	if (config.logUrl) {
		detailLines.push(`- Full workflow logs: ${toLink(config.logUrl, config.logUrl)}`)
	}

	if (config.logExcerpt) {
		detailLines.push('', '```text', sanitizeCodeFenceContent(config.logExcerpt), '```')
	}
...
async function upsertPrComment(config, prNumber) {
	...
	const existingComment = Array.isArray(comments)
		? comments.find(
				(comment) => typeof comment.body === 'string' && comment.body.includes(config.commentMarker)
			)
		: undefined
...
async function createDeployment(config) {
	...
	const deployment = await githubRequest(
		...
		`/repos/${config.owner}/${config.repo}/deployments`,
		{
			...
			task: config.deploymentKind === 'production' ? 'deploy' : 'deploy:preview',
			auto_merge: false,
			required_contexts: [],
...
function matchesCleanupTarget(config, deployment) {
	...
	if (config.refName && payload.refName && payload.refName !== config.refName) {
		return false
	}

	if (config.commentKey && payload.commentKey && payload.commentKey !== config.commentKey) {
		return false
	}
...
async function deactivateDeployments(config) {
	...
	const matchingDeployments = Array.isArray(deployments)
		? deployments.filter((deployment) => matchesCleanupTarget(config, deployment))
		: []
...
export function buildConfig() {
	...
	const environmentUrl =
		getOptionalInput('environment-url') ??
	...
	return {
		...
		transientEnvironment: getBooleanInput('transient-environment', deploymentKind !== 'production'),
		productionEnvironment: getBooleanInput(
			'production-environment',
			deploymentKind === 'production'
		),
		ignoreCommentPermissionErrors: getBooleanInput('ignore-comment-permission-errors', true)
	}
}
...
async function runCommentFeedback(config) {
	...
	throw new Error('Comment feedback requires pr-number or resolve-pr-from-ref with ref-name')
}

function handleCommentFeedbackFailure(config, error, failures) {
	if (config.ignoreCommentPermissionErrors && isCommentPermissionError(error)) {
		warn(
