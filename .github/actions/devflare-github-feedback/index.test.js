...
describe("grouped comment rendering", () => {
	function createConfig(overrides = {}) {
		return {
			...
			commentGroupSummary:
				"This single comment tracks the latest documentation and testing preview results for the pull request.",
			...overrides,
		};
	}
	...
	test("renders grouped comments with multiple workflow sections", () => {
		...
		expect(body).toContain(
			"### ✅ Testing PR preview deployed successfully",
		);
		expect(body.indexOf("Documentation PR preview")).toBeLessThan(
			body.indexOf("Testing PR preview"),
		);
	});
	...
	test("parses grouped comment sections from an existing shared comment body", () => {
		...
		const sections = parseGroupedCommentSections(
			"pr-deployment-status",
			body,
		);

		expect(sections.get("documentation-preview")).toContain(
			"Documentation section",
		);
