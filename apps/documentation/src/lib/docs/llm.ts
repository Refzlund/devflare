...
function renderLabeledBlock(
	...
	return joinBlocks([createHeading(level, title), body])
}
...
function renderFactsTable(doc: DocPage): string {
	return [
		...
		'| --- | --- |',
		...doc.facts.map(
			(fact) => `| ${escapeTableCell(fact.label)} | ${escapeTableCell(fact.value)} |`
		)
	].join('\n')
}

function renderSection(section: DocSection, level = 4): string {
...
function getUniqueOrderedDocs(): DocPage[] {
	const orderedDocs = docGroups.flatMap((group) =>
	...
	for (const group of docGroups) {
		...
		for (const category of group.categories) {
			if (category.sidebarDisplay === 'standalone') {
				lines.push(
					...category.items.map(
						(item) => `- [${item.navTitle}](${docPath(item.slug)}) — ${item.summary}`
					)
				)
				continue
			}
...
export function buildStrictLLMDocument(): string {
	const strictDocs = getUniqueOrderedDocs().filter(
		(doc) => !STRICT_EXCLUDED_DOC_SLUGS.has(doc.slug)
	)
